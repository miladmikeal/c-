...
int Medusa::attack() {
    ...
    // Otherwise
        ...
        // Normal damage
        cout << "Medusa attacked. " << damage << " damage points "
             << "generated." << endl;
    }
    // Return damage
    return damage;
...
void Medusa::defense(int d) {
    ...
    if (d != 1000) {
        ...
        if (damage <= 0) {
            // No damage taken
            cout << "Medusa's defense prevails. "
                 << "0 damage taken." << endl
                 << "Medusa's remaining strength: "
                 << getStrength() << endl;
        } else if (damage > 0) {
            // Factor in damage
            ...
            if (getStrength() > 0) {
                cout << "Medusa has been damaged " << damage
    ...
    // If Medusa turned to stone by other Medusa
        ...
        // This Medusa dies
        cout << "Medusa looked into other Medusa's eyes and died!" << endl;
        setDead(true);
    }
}
